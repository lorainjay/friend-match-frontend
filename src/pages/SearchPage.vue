<template>
  ...
  <van-row gutter="16" style="padding: 0 16px">
    <van-col v-if="userTags" v-for="tag in userTags">
      <van-tag  closeable size="small" type="primary" @close="doClose(tag)">
        {{ tag }}
      </van-tag>
    </van-col>

    <van-col v-else v-for="tag in activeIds">
      <van-tag  closeable size="small" type="primary" @close="doClose(tag)">
        {{ tag }}
      </van-tag>
    </van-col>
  ...
  <van-tree-select v-if="userId"
      v-model:active-id= "userTags"
      v-model:main-active-index="activeIndex"
      :items="tagList"
  />
  <van-tree-select v-else
...
<script setup>
    ...
    const updateTag = async () => {
      let res = await myAxios.put("/user/update/tags", {
          id: userId.value,
          tagList: userTags.value
      },
      );
      if (res?.code === 0) {
    ...
    const doClose = tag =>{
      userTags.value  = userTags.value.filter(item => {
        return tag !== item;//不等于要删的内容才保留
      })
    }
